...
<div class="pcard">
    ...
    <div class="pstats">
        ...
        <div class="pstat"><div class="pstat-num">{{ $artikelDibaca }}</div><div class="pstat-label">Artikel Dibaca</div></div>
...
<div class="sec">
    ...
    <div class="sec-body" id="body-pw">
        <form method="POST" action="{{ route('profil.password') }}">@csrf
            <div class="form-group"><label class="form-label">Password Saat Ini</label><input type="password" name="current_password" class="form-control @error('current_password') is-invalid @enderror" placeholder="Masukkan password lama">@error('current_password')<span class="invalid-feedback">{{ $message }}</span>@enderror</div>
            <div class="form-group"><label class="form-label">Password Baru</label><input type="password" name="password" class="form-control @error('password') is-invalid @enderror" placeholder="Min. 8 karakter">@error('password')<span class="invalid-feedback">{{ $message }}</span>@enderror</div>
...
<div class="sec">
    ...
    <div class="sec-body" id="body-lang">
        <form method="POST" action="{{ route('profil.bahasa') }}">@csrf
            <p style="font-size:.875rem;color:var(--text-muted);margin-bottom:14px;">Pilih bahasa yang ingin Anda gunakan:</p>
            <label class="lang-opt {{ $user->bahasa=='id' ? 'sel' : '' }}" id="lo-id" onclick="selLang('id')">
                <input type="radio" name="bahasa_r" {{ $user->bahasa=='id' ? 'checked' : '' }}>
            ...
            <label class="lang-opt {{ $user->bahasa=='en' ? 'sel' : '' }}" id="lo-en" onclick="selLang('en')">
                <input type="radio" name="bahasa_r" {{ $user->bahasa=='en' ? 'checked' : '' }}>
                <span style="font-size:1.2rem;">🇬🇧</span>
            ...
            <input type="hidden" name="bahasa" id="bahasa-val" value="{{ $user->bahasa }}">
...
@if($errors->has('name')||$errors->has('email')) document.getElementById('sec-edit').style.display='block'; @endif
@if($errors->has('current_password')||$errors->has('password')) document.getElementById('body-pw').classList.add('open');document.getElementById('chev-pw').style.transform='rotate(180deg)'; @endif
</script>
@endsection
